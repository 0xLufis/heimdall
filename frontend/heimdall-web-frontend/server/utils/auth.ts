...
const authSecret = process.env.BETTER_AUTH_SECRET || (process.env.NODE_ENV !== 'production' ? "heimdall-local-dev-secret-key-32-chars-min-security" : "");
if (!authSecret) {
   throw new Error("CRITICAL SECURITY CONFIGURATION ERROR: BETTER_AUTH_SECRET must be defined in production.");
}
